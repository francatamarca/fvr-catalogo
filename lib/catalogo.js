...
export async function getCatalogo() {
  ...
  // dev: preferí archivo local si existe
  ...
  try {
    ...
    cache = JSON.parse(gunzipSync(buf).toString('utf8'));
  } catch {
...
function safePrecio(pr) {
  ...
  return {
    via: 'general', precio_unit_usd: pr.precio_unit_usd, flete_usd: pr.flete_usd,
    flete_consultar: !!pr.flete_consultar, flete_estimado: !!pr.flete_estimado, combo: pr.combo,
  };
}
export function clientProduct(p, full = false) {
  ...
  if (full) { base.imgs = p.imgs; base.specs = p.specs; base.pesoKg = p.pesoKg; }
  return base;
}
